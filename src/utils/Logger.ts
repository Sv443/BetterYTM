...
export class Logger {
  ...
  static #pushLog(category: string, type: string, time?: number, ...args: unknown[]) {
  ...
  static #serializeLogVal(val: unknown, primaryScope = true): string {
  ...
  static #getLogLevel(args: unknown[]): number {
  ...
  errorNoToast(...args: unknown[]): void {
    Logger.#pushLog(this.category, "ERROR", Date.now(), ...args);
    console.error(this.consPrefix, ...args);
  }
