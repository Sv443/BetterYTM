...
const onlyLocalesRaw = process.argv.find((v) => v.startsWith("--only") || v.toLowerCase().match(/^-o[\s=]/));
const onlyLocales = onlyLocalesRaw?.split(/[\s=]/)[1]?.replace(/"/g, "")
  ?.replace(/\s/g, "")?.split(",") as TrLocale[] | undefined;

const onlyKeysRaw = process.argv.find((v) => v.startsWith("--keys") || v.toLowerCase().match(/^-k[\s=]/));
const onlyKeys = onlyKeysRaw?.split(/[\s=]/)[1]?.replace(/"/g, "").split(",").map(k => k.trim()) as string[] | undefined;

const includeBased = Boolean(process.argv.some((v) => v.match(/--include-based/) || v.toLowerCase() === "-b"));
