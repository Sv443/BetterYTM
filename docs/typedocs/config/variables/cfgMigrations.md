...
[@sv443/betterytm](../../modules.md) / [config](../README.md) / cfgMigrations

# Variable: cfgMigrations

> `const` **cfgMigrations**: `DataMigrationsDict`

Defined in: [src/config.ts:42](https://github.com/Sv443/BetterYTM/blob/6955ce00e166b535e9519c862d94f5c834e79ebb/src/config.ts#L42)
...
- **When a feature's valid value range was changed,** the migration function should use `useNewRanges()` to clamp the feature's value to the new valid range. This only applies to numeric features with a `min` and `max` property defined in the `featInfo` object.
