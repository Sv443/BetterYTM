...
(function(){'use strict';// lib/math.ts
...
function randRange$1(...args) {
  ...
  if (enhancedEntropy) {
    ...
    return Number(Array.from(
      uintArr,
      (v) => Math.round(mapRange$1(v, 0, 255, min, max)).toString(10)
    ).join(""));
  } else
...
function valsWithin$1(a, b, dec = 1, withinRange = 0.5) {
...
function darkenColor$1(color, percent, upperCase = false) {
  ...
  if (isHexCol)
    return rgbToHex$1(r, g, b, a, color.startsWith("#"), upperCase);
...
function hexToRgb$1(hex) {
  ...
  const b = hexInt & 255;
  return [clamp$1(r, 0, 255), clamp$1(g, 0, 255), clamp$1(b, 0, 255), typeof a === "number" ? clamp$1(a, 0, 1) : void 0];
}
...
function randomId$1(length = 16, radix = 16, enhancedEntropy = false, randomCase = true) {
  ...
  if (enhancedEntropy) {
    ...
    arr = Array.from(
      uintArr,
      (v) => mapRange$1(v, 0, 255, 0, radix).toString(radix).substring(0, 1)
    );
  } else {
    arr = Array.from(
      { length },
...
// lib/Errors.ts
var DatedError$1 = class DatedError extends Error {
  date;
...
var ChecksumMismatchError$1 = class ChecksumMismatchError extends DatedError$1 {
  constructor(message, options) {
    super(message, options);
    this.name = "ChecksumMismatchError";
  }
};
var CustomError$1 = class CustomError extends DatedError$1 {
  constructor(name, message, options) {
    super(message, options);
    this.name = name;
  }
};
var MigrationError$1 = class MigrationError extends DatedError$1 {
  constructor(message, options) {
    super(message, options);
    this.name = "MigrationError";
  }
};
var ValidationError$1 = class ValidationError extends DatedError$1 {
  constructor(message, options) {
    super(message, options);
    this.name = "ValidationError";
  }
};
var ScriptContextError$1 = class ScriptContextError extends DatedError$1 {
  constructor(message, options) {
    super(message, options);
    this.name = "ScriptContextError";
  }
};
var NetworkError$1 = class NetworkError extends DatedError$1 {
  constructor(message, options) {
    super(message, options);
...
function scheduleExit$1(code = 0, timeout = 0) {
  ...
  if (typeof process !== "undefined" && "exit" in process && typeof process.exit === "function")
    ...
    throw new ScriptContextError$1("Cannot exit the process, no exit method available");
...
// lib/NanoEmitter.ts
var NanoEmitter$2 = class NanoEmitter {
...
// lib/DataStore.ts
var dsFmtVer$1 = 1;
var DataStore$1 = class DataStore extends NanoEmitter$2 {
  id;
  formatVersion;
  defaultData;
  encodeData;
  ...
  async loadData() {
    try {
      if (this.firstInit) {
        ...
        if (isNaN(dsVer) || dsVer < dsFmtVer$1)
          await this.engine.setValue("__ds_fmt_ver", dsFmtVer$1);
      }
  ...
  getData() {
    if (!this.memoryCache)
      throw new DatedError$1("In-memory cache is disabled for this DataStore instance, so getData() can't be used. Please use loadData() instead.");
...
var DataStoreEngine$1 = class DataStoreEngine {
  dataStoreOptions;
  ...
  ensureDataStoreOptions() {
    if (!this.dataStoreOptions)
      throw new DatedError$1("DataStoreEngine must be initialized with DataStore options before use. If you are using this instance standalone, set them in the constructor or call `setDataStoreOptions()` with the DataStore options.");
    if (!this.dataStoreOptions.id)
      throw new DatedError$1("DataStoreEngine must be initialized with a valid DataStore ID");
  }
  /**
...
var BrowserStorageEngine$1 = class BrowserStorageEngine extends DataStoreEngine$1 {
  options;
  /**
...
var fs$1;
var FileStorageEngine$1 = class FileStorageEngine extends DataStoreEngine$1 {
  options;
  ...
  async readFile() {
    ...
    try {
      if (!fs$1)
        fs$1 = (_a = await import('fs/promises')) == null ? void 0 : _a.default;
      if (!fs$1)
        throw new ScriptContextError$1("FileStorageEngine requires Node.js or Deno with Node compatibility (v1.31+)", { cause: new DatedError$1("'node:fs/promises' module not available") });
...
var DataStoreSerializer$1 = class _DataStoreSerializer {
  ...
  constructor(stores, options = {}) {
    if (!crypto || !crypto.subtle)
      throw new ScriptContextError$1("DataStoreSerializer has to run in a secure context (HTTPS) or in another environment that implements the subtleCrypto API!");
    this.stores = stores;
    this.options = {
      addChecksum: true,
  ...
  async deserializePartial(stores, data) {
    ...
    for (const storeData of deserStores) {
      ...
      if (!storeInst)
        throw new DatedError$1(`Can't deserialize data because no DataStore instance with the ID "${effectiveId}" was found! Make sure to provide it in the DataStoreSerializer constructor.`);
      if (this.options.ensureIntegrity && typeof storeData.checksum === "string") {
        const checksum = await this.calcChecksum(storeData.data);
        if (checksum !== storeData.checksum)
          throw new ChecksumMismatchError$1(`Checksum mismatch for DataStore with ID "${storeData.id}"!
...
// lib/Debouncer.ts
var Debouncer$1 = class Debouncer extends NanoEmitter$2 {
  /**
...
function debounce$1(fn, timeout = 200, type = "immediate", nanoEmitterOptions) {
  ...
  return func;
}var CoreUtils=/*#__PURE__*/Object.freeze({__proto__:null,BrowserStorageEngine:BrowserStorageEngine$1,ChecksumMismatchError:ChecksumMismatchError$1,CustomError:CustomError$1,DataStore:DataStore$1,DataStoreEngine:DataStoreEngine$1,DataStoreSerializer:DataStoreSerializer$1,DatedError:DatedError$1,Debouncer:Debouncer$1,FileStorageEngine:FileStorageEngine$1,MigrationError:MigrationError$1,NanoEmitter:NanoEmitter$2,NetworkError:NetworkError$1,ScriptContextError:ScriptContextError$1,ValidationError:ValidationError$1,abtoa:abtoa$1,atoab:atoab$1,autoPlural:autoPlural$2,bitSetHas:bitSetHas$1,capitalize:capitalize$1,clamp:clamp$1,compress:compress$1,computeHash:computeHash$1,consumeGen:consumeGen$1,consumeStringGen:consumeStringGen$1,createProgressBar:createProgressBar$1,createRecurringTask:createRecurringTask$1,createTable:createTable$1,darkenColor:darkenColor$1,debounce:debounce$1,decompress:decompress$1,defaultPbChars:defaultPbChars$1,defaultTableLineCharset:defaultTableLineCharset$1,digitCount:digitCount$1,fetchAdvanced:fetchAdvanced$1,formatNumber:formatNumber$2,getCallStack:getCallStack$1,getListLength:getListLength$1,getterifyObj:getterifyObj$2,hexToRgb:hexToRgb$1,insertValues:insertValues$1,joinArrayReadable:joinArrayReadable$1,lightenColor:lightenColor$1,mapRange:mapRange$1,overflowVal:overflowVal$2,pauseFor:pauseFor$1,pureObj:pureObj$2,randRange:randRange$1,randomId:randomId$1,randomItem:randomItem$1,randomItemIndex:randomItemIndex$1,randomizeArray:randomizeArray$1,rgbToHex:rgbToHex$1,roundFixed:roundFixed$1,scheduleExit:scheduleExit$1,secsToTimeStr:secsToTimeStr$1,setImmediateInterval:setImmediateInterval$1,setImmediateTimeoutLoop:setImmediateTimeoutLoop$1,takeRandomItem:takeRandomItem$1,takeRandomItemIndex:takeRandomItemIndex$1,truncStr:truncStr$1,valsWithin:valsWithin$1});var __defProp = Object.defineProperty;
var __defNormalProp = (obj, key, value) => key in obj ? __defProp(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
var __publicField = (obj, key, value) => __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);

// node_modules/.pnpm/@sv443-network+coreutils@3.6.0/node_modules/@sv443-network/coreutils/dist/CoreUtils.mjs
...
var NanoEmitter$1 = class NanoEmitter {
  /** Creates a new instance of NanoEmitter - a lightweight event emitter with helper methods and a strongly typed event map */
  constructor(options = {}) {
...
// lib/consts.ts
var rawConsts$1 = {
  coreUtilsVersion: "3.6.0",
  userUtilsVersion: "10.4.0"
};
function getConst$1(constKey, defaultVal) {
  const val = rawConsts$1[constKey];
  return val.match(/^#\{\{.+\}\}$/) ? defaultVal : val;
}
var versions = {
  /** Semver version string of the bundled library CoreUtils. */
  CoreUtils: getConst$1("coreUtilsVersion", "ERR:unknown"),
  /** Semver version string of UserUtils. */
  UserUtils: getConst$1("userUtilsVersion", "ERR:unknown")
};
...
function interceptEvent(eventObject, eventName, predicate = () => true) {
  var _a;
  if (typeof window.GM === "object" && ((_a = GM == null ? void 0 : GM.info) == null ? void 0 : _a.scriptHandler) && GM.info.scriptHandler === "FireMonkey" && (eventObject === window || eventObject === getUnsafeWindow$1()))
...
function setInnerHtmlUnsafe(element, html) {
  var _a, _b, _c;
  if (!ttPolicy$1 && typeof ((_a = window == null ? void 0 : window.trustedTypes) == null ? void 0 : _a.createPolicy) === "function") {
    ttPolicy$1 = window.trustedTypes.createPolicy("_uu_set_innerhtml_unsafe", {
...
var Dialog = class _Dialog extends NanoEmitter$1 {
  ...
  async open(e) {
    ...
    if (openDialogs$1.includes(this.id))
    ...
    for (const dialogId of openDialogs$1)
      if (dialogId !== this.id)
  ...
  close(e) {
    ...
    if (currentDialogId$1)
      (_b = document.querySelector(`#uu-${currentDialogId$1}-dialog-bg`)) == null ? void 0 : _b.removeAttribute("inert");
...
};var UserUtils=/*#__PURE__*/Object.freeze({__proto__:null,BrowserStorageEngine:BrowserStorageEngine,ChecksumMismatchError:ChecksumMismatchError,CustomError:CustomError,DataStore:DataStore,DataStoreEngine:DataStoreEngine,DataStoreSerializer:DataStoreSerializer,DatedError:DatedError,Debouncer:Debouncer,Dialog:Dialog,FileStorageEngine:FileStorageEngine,GMStorageEngine:GMStorageEngine,MigrationError:MigrationError,Mixins:Mixins,NanoEmitter:NanoEmitter$1,NetworkError:NetworkError,PlatformError:PlatformError,ScriptContextError:ScriptContextError,SelectorObserver:SelectorObserver,ValidationError:ValidationError,abtoa:abtoa,addGlobalStyle:addGlobalStyle,addParent:addParent,atoab:atoab,autoPlural:autoPlural$1,bitSetHas:bitSetHas,capitalize:capitalize,clamp:clamp,compress:compress,computeHash:computeHash,consumeGen:consumeGen,consumeStringGen:consumeStringGen,createProgressBar:createProgressBar,createRecurringTask:createRecurringTask,createTable:createTable,get currentDialogId(){return currentDialogId$1},darkenColor:darkenColor,debounce:debounce,decompress:decompress,defaultDialogCss:defaultDialogCss,defaultPbChars:defaultPbChars,defaultStrings:defaultStrings,defaultTableLineCharset:defaultTableLineCharset,digitCount:digitCount,fetchAdvanced:fetchAdvanced,formatNumber:formatNumber$1,getCallStack:getCallStack,getListLength:getListLength,getSiblingsFrame:getSiblingsFrame,getUnsafeWindow:getUnsafeWindow$1,getterifyObj:getterifyObj$1,hexToRgb:hexToRgb,insertValues:insertValues,interceptEvent:interceptEvent,interceptWindowEvent:interceptWindowEvent,isDomLoaded:isDomLoaded,isScrollable:isScrollable,joinArrayReadable:joinArrayReadable,lightenColor:lightenColor,mapRange:mapRange,observeElementProp:observeElementProp,onDomLoad:onDomLoad$1,openDialogs:openDialogs$1,openInNewTab:openInNewTab,overflowVal:overflowVal$1,pauseFor:pauseFor,preloadImages:preloadImages,probeElementStyle:probeElementStyle,pureObj:pureObj$1,randRange:randRange,randomId:randomId,randomItem:randomItem,randomItemIndex:randomItemIndex,randomizeArray:randomizeArray,rgbToHex:rgbToHex,roundFixed:roundFixed,scheduleExit:scheduleExit,secsToTimeStr:secsToTimeStr,setImmediateInterval:setImmediateInterval,setImmediateTimeoutLoop:setImmediateTimeoutLoop,setInnerHtmlUnsafe:setInnerHtmlUnsafe,takeRandomItem:takeRandomItem,takeRandomItemIndex:takeRandomItemIndex,tr:tr,truncStr:truncStr,valsWithin:valsWithin,versions:versions});/*! @license DOMPurify 3.3.3 | (c) Cure53 and other contributors | Released under the Apache license 2.0 and Mozilla Public License 2.0 | github.com/cure53/DOMPurify/blob/3.3.3/LICENSE */
...
var purify = createDOMPurify();const semver = /^[v^~<>=]*?(\d+)(?:\.([x*]|\d+)(?:\.([x*]|\d+)(?:\.([x*]|\d+))?(?:-([\da-z\-]+(?:\.[\da-z\-]+)*))?(?:\+[\da-z\-]+(?:\.[\da-z\-]+)*)?)?)?$/i;
...
const compare = (v1, v2, operator) => {
    ...
    const res = compareVersions$1(v1, v2);
...
const validateStrict = (version) => typeof version === 'string' &&
    /^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-((?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*)(?:\.(?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*))*))?(?:\+([0-9a-zA-Z-]+(?:\.[0-9a-zA-Z-]+)*))?$/.test(version);var compareVersions=/*#__PURE__*/Object.freeze({__proto__:null,compare:compare,compareVersions:compareVersions$1,satisfies:satisfies,validate:validate,validateStrict:validateStrict});var preloadAssetPattern = "^(icon|img)-";
var resources = {
...
async function fetchTranslationResource(locale) {
    ...
    const res = await fetchAdvanced$1(url);
...
class BytmDialog extends NanoEmitter$2 {
    //#region constructor
    constructor(options) {
        super();
...
const siteEvents = new NanoEmitter$2({
    publicEmit: true,
});
let observers = [];
let lastVidId = null;
    ...
    try {
        const logEmit = () => {
            if (getFeature("logEvents")) {
                args.length > 0
                    ? log(`Emitted site event 'bytm:siteEvent:${key}' with ${args.length} ${autoPlural$2("argument", args)}:`, ...args)
    ...
    async renderBody({ type, message, ...rest }) {
        ...
        if (type === "prompt") {
            ...
            inputElem.value = "defaultValue" in rest && rest.defaultValue
                ? await consumeStringGen$1(rest.defaultValue)
                : "";
...
async function initVolumeFeatures() {
    ...
    window.addEventListener("resize", debounce$1(onResize, Math.floor(1000 / 6)));
...
// @ts-expect-error - no idea why HTMLMediaElement wouldn't exist on Window
 } = Object.getOwnPropertyDescriptor(getUnsafeWindow$1().HTMLMediaElement.prototype, "volume") ?? {};
...
const getLogsTxt = () => {
    ...
    const getVal = (val, primaryScope = true) => {
        ...
        if (val instanceof DatedError$1)
...
function getLogLevel(args) {
    ...
    if (typeof lastArg === "number" && lastArg >= 0 && lastArg <= (Object.keys(LogLevel).length / 2) - 1)
        return clamp$1(args.splice(args.length - 1)[0], minLogLvl, maxLogLvl);
...
const showErrToast = debounce$1((errName, ...args) => showIconToast({
...
class LyricsError extends DatedError$1 {
...
class PluginError extends DatedError$1 {
    constructor(message, opts) {
        super(message, opts);
...
async function initRememberVideoTime() {
    ...
    if (remTimeEntries.some(e => "watchID" in e)) {
        ...
        log(`Removed ${remTimeEntries.length} ${autoPlural$2("entry", remTimeEntries)} with an outdated format from the video time cache`);
    }
    log(`Initialized video time restoring with ${remTimeEntries.length} initial ${autoPlural$2("entry", remTimeEntries)}:`, remTimeEntries);
...
async function fetchCss(key) {
    try {
        const css = await (await fetchAdvanced$1(await getResourceUrl(key))).text();
        ...
        exportPane.classList.add("bytm-exim-dialog-pane", "export");
        {
            ...
            const copyBtn = await createLongBtn({
                ...
                async onClick({ shiftKey }) {
                    ...
                    copyToClipboard(await consumeStringGen$1(copyData));
    ...
    if (!autoLikeExImDialog) {
        autoLikeExImDialog = new ExImDialog({
            ...
            exportData: async () => await compressionSupported()
                ? await compress$1(JSON.stringify(autoLikeStore.getData()), compressionFormat$1, "string")
...
const autoLikeStore = new DataStore$1({
...
async function fetchLyricsUrls(artist, song) {
    try {
        ...
        const fetchRes = await fetchAdvanced$1(fetchUrl, {
            ...(token ? {
                headers: {
...
async function mountCfgMenu() {
    try {
        ...
        const exImDlg = new ExImDialog({
            ...
            // try to compress the data if possible
            exportData: async () => await compressionSupported()
                ? await compress$1(JSON.stringify({ formatVersion: cfgFormatVersion, data: getFeatures() }), compressionFormat$1, "string")
                : exportDataSpecial(),
            exportDataSpecial,
            async onImport(data) {
                try {
        ...
        const onCfgChange = async (key, initialVal, newVal) => {
            ...
            if (["number", "slider"].includes(ftInfo.type)) {
                ...
                if (("min" in ftInfo || "max" in ftInfo))
                    newVal = clamp$1(Number(newVal), "min" in ftInfo ? Number(ftInfo.min) : -Infinity, "max" in ftInfo ? Number(ftInfo.max) : Infinity);
        ...
        window.addEventListener("resize", debounce$1(checkToggleScrollIndicator, 250));
...
function improveLogo() {
    return new Promise(async (resolve) => {
        try {
            ...
            const res = await fetchAdvanced$1("https://music.youtube.com/img/on_platform_logo_dark.svg");
    ...
    try {
        ...
        addSelectorListener("sideBar", "#contentContainer #guide-content #items ytmusic-guide-entry-renderer", {
            listener: (sidebarCont) => {
                ...
                log(`Added anchors around ${itemsAmt} sidebar ${autoPlural$2("item", itemsAmt)}`);
            },
        });
        addSelectorListener("body", "ytmusic-nav-bar", {
            listener(navBar) {
                ...
                const improveMiniSidebarAnchors = () => {
                    ...
                    log(`Added anchors around ${itemsAmt} mini sidebar ${autoPlural$2("item", itemsAmt)}`);
                    mut.disconnect();
                };
                if (miniSidebarCont)
                    improveMiniSidebarAnchors();
            ...
            addSelectorListener("sidePanel", "ytmusic-player-queue #contents, ytmusic-player-queue #automix-contents", {
                ...
                listener(songLists) {
                    songLists.forEach((songListEl) => {
                        ...
                        itemsAmt > 0 && log(`Improved clickable area of ${itemsAmt} current song list ${autoPlural$2("item", itemsAmt)}`);
                    });
                },
            });
        };
        ...
        const genericSongListListener = (songLists) => {
            songLists.forEach((songListEl) => {
                ...
                if (!items.length)
                    return;
                const itemsAmt = improveSongListClickArea(items);
                itemsAmt > 0 && log(`Improved clickable area of ${itemsAmt} song list ${autoPlural$2("item", itemsAmt)}`);
            });
        };
...
/** Album artwork cache */
const artCacheStore = new DataStore$1({
...
async function getStaticData() {
    try {
        if (staticData)
            return staticData;
        const res = await fetchAdvanced$1(remoteDataUrl, {
            timeout: 10000,
        });
        if (res.ok) {
...
const alertsStore = new DataStore$1({
    id: "bytm-alerts",
    defaultData: {
        dismissed: [],
    },
...
function isAlertActive(alert, alertsData) {
    ...
    if ("version" in alert && alert.version !== scriptInfo$1.version)
        return false;
    if ("versionMin" in alert && alert.versionMin && compareVersions$1(alert.versionMin, scriptInfo$1.version) > 0)
        return false;
    if ("versionMax" in alert && alert.versionMax && compareVersions$1(alert.versionMax, scriptInfo$1.version) < 0)
        return false;
...
function getDSSerializer(full = false) {
    ...
    if (!full)
        return serializer ?? (serializer = new DataStoreSerializer$1(getSerializerStores(), dsOpts));
    else
        return fullSerializer ?? (fullSerializer = new DataStoreSerializer$1(getSerializerStoresFull(), dsOpts));
}
/**
...
const broadcastTxID = randomId$1(10, 36);
 ...
 */
const broadcastEng = new GMStorageEngine({ dataStoreOptions: broadcastEngDSOpts });
...
function relayBroadcastPacket(packet) {
    ...
    const packetClean = pureObj$2(packet); // remove prototype chain
...
function getSessionId() {
    try {
        ...
        if (!sesId)
            window.sessionStorage.setItem("_bytm-session-id", sesId = randomId$1(10, 36));
        return sesId;
    }
    catch (err) {
...
const reloadTabStore = new DataStore$1({
    id: "bytm-reload-tab",
    engine: new GMStorageEngine(),
    formatVersion: 0,
    compressionFormat: null,
...
async function reloadAllTabs(reloadSelf = true, toTxIDs) {
    ...
    return reloadSelf
        ? await (async () => {
            await pauseFor$1(30); // broadcast is synchronous, but we might still be working on something in our async queue
...
const resourceCacheStore = new DataStore$1({
...
async function renderBody$1() {
    ...
    for (const [, { def: { plugin, intents: intentsRaw } }] of registeredPlugins) {
        ...
        const intentsArr = bitSetHas$1(intentsBitSet, PluginIntent.FullAccess)
...
async function initArrowKeySkip() {
    ...
    document.addEventListener("keydown", (evt) => {
        ...
        if (vidElem && vidElem.readyState > 0)
            vidElem.currentTime = clamp$1(vidElem.currentTime + skipBy, 0, vidElem.duration);
    });
...
function handleVolumeKeyPress(evt) {
    ...
    const newVol = clamp$1(Number(sliderEl.value)
        + (evt.code === "ArrowUp" ? 1 : -1)
            * clamp$1((getFeature("arrowKeyVolumeStep", featInfo.arrowKeyVolumeStep.default)), isNaN(step) ? 5 : step, 100), 0, 100);
...
async function initQueueButtons() {
    ...
    const tryAddCurrentQueueBtns = (parentSelector) => {
        ...
        if (amt > 0)
            log(`Added buttons to ${amt} new queue ${autoPlural$2("item", amt)}`);
    };
    // current queue
    ...
    if (queueItems.length > 0) {
        ...
        log(`Added buttons to ${queueItems.length} existing "current song queue" ${autoPlural$2("item", queueItems)}`);
    }
    ...
    const tryAddGenericListQueueBtns = (listElem) => {
        ...
        addedBtnsCount > 0 &&
            log(`Added buttons to ${addedBtnsCount} new "generic song list" ${autoPlural$2("item", addedBtnsCount)} in list`, listElem);
    };
...
class ExampleError extends DatedError$1 {
    constructor(message, options) {
        super(message, options);
...
async function getAdornHtml(className, title, resource, extraAttributes, clickDialogText) {
    title = title ? await consumeStringGen$1(title) : undefined;
    extraAttributes = extraAttributes ? await consumeStringGen$1(extraAttributes) : undefined;
...
async function resolveAdornments(ftInfo, featKey) {
    ...
    if (feat.since && compare(feat.since, scriptInfo$1.version, isDev ? ">" : ">=") && (getVersionSessionCount() < newFeatureAdornmentMaxSessionCount || isDev))
...
const cfgDefaultData = pureObj$2(Object.keys(featInfo)
...
const cfgMigrations = {
    ...
    7: (oldData) => {
        ...
        newData.arrowKeySkipBy = clamp$1(newData.arrowKeySkipBy, 0.5, 30);
        return newData;
    },
    // 7 -> 8 (v2.1)
    8: (oldData) => {
...
//#region >> store
const configStore = new DataStore$1({
    id: "bytm-config",
...
async function initConfig() {
    ...
    try {
        ...
        if (oldDataObj !== null && typeof oldDataObj === "object" && Object.keys(oldDataObj).length > 0)
            oldDataHash = await computeHash$1(JSON.stringify(oldDataObj), "sha256");
    }
    catch {
    }
...
let devPluginToken;
const devPluginId = randomId$1(8, 36, true, true);
...
function parseBitSetEnumArray(bitSet, enumRef) {
    ...
    for (const [, val] of Object.entries(enumRef))
        if ((typeof val === "number" || typeof val === "bigint") && bitSetHas$1(bitSet, val))
            result.push(val);
    return result;
}
...
function validatePluginDef(pluginDef) {
    ...
    else if (!validateStrict(plugin.version))
...
const tempTargetAttrName = `data-tmp-target-${randomId$1(6, 36)}`;
...
async function onDomLoad() {
    ...
    try {
        ...
        getUnsafeWindow$1().dispatchEvent(new Event("resize", { bubbles: true, cancelable: true }));
        // preload icons
        preloadResources();
        ...
        for (const key of keys) {
            try {
                ...
                values[key] = typeof val !== "undefined" && isEncoded
                    ? await decompress$1(val, compressionFormat$1, "string")
                    : val;
            ...
            extraButtons: [
                (dlg) => {
                    ...
                    btn.addEventListener("click", async () => {
                        ...
                        try {
                            if (val && val.length > 0) {
                                const result = await compress$1(val, compressionFormat$1);
                                dlg.emitResolve(result);
                                dlg.close();
                ...
                (dlg) => {
                    ...
                    btn.addEventListener("click", async () => {
                        ...
                        try {
                            if (val && val.length > 0) {
                                const result = await decompress$1(val, compressionFormat$1);
                                dlg.emitResolve(result);
        ...
        setTimeout(() => {
            ...
            console.log(`[${scriptInfo$1.name}/#DEBUG] Collected information from ${sessions.length} open ${autoPlural$2("tab", sessions)}:\n${createTable$1([
                ...
                ...sessions.map(([txID, { sessionId, title, domain, initTime }], i) => {
                    const initSince = secsToTimeStr$1(Math.floor((Date.now() - initTime) / 1000)).padStart(5, "0");
                    return [
                        i + 1,
