...
(function(_sv443_network_userutils, marked, dompurify, compare_versions) {
	...
	*/
	function getString(id, throws) {
		if (throws !== false) try {
			if (typeof staticData?.strings !== "object") throw new _sv443_network_userutils.DatedError("Static data hasn't been fetched yet.");
			const str = staticData.strings?.[id];
			if (typeof str !== "string") throw new _sv443_network_userutils.DatedError(`String '${id}' doesn't exist or is not of type string.`);
			return str;
		} catch (e) {
		...
		try {
			...
			switch (getDomain()) {
				case "ytm": {
					...
					globservers.browseResponse = new _sv443_network_userutils.SelectorObserver(browseResponseSelector, {
						...defaultObserverOptions,
						defaultDebounce: (0, _sv443_network_userutils.clamp)(Math.floor(defaultObserverOptions.defaultDebounce / 2), 50, 1e3),
						subtree: true
					});
			...
			for (const [name, obs] of Object.entries(globservers)) {
				...
				obs.on("found", (data) => {
					...
					if (getFeature("verboseObservers")) loggers.debug.info(`SelectorObserver with name '${name}' and base element ${baseElem} found ${elements.length} ${(0, _sv443_network_userutils.autoPlural)("element", elements)}:`, elements.join(", "), LogLevel.Info);
...
ytmusic-section-list-renderer[page-type="MUSIC_PAGE_TYPE_PLAYLIST"] ytmusic-shelf-renderer #contents`, {
	...
	*/
	...
	async function initBeforeUnloadHook() {
		try {
			(0, _sv443_network_userutils.interceptWindowEvent)("beforeunload", () => getDiscardOverride() ?? getFeature("disableBeforeUnloadPopup"));
		} catch (err) {
	...
	function preInit() {
		try {
			...
			if (["FireMonkey"].includes(GM.info?.scriptHandler ?? "")) {
				...
				if ((0, _sv443_network_userutils.isDomLoaded)()) alert(msg);
				else (0, _sv443_network_userutils.onDomLoad)().then(() => alert(msg));
				return;
			}
