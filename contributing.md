...
- Plugin Development:
  ...
  - [**Almanac (explanation of terms and concepts)**](./docs/almanac.md)
  - [**View autogenerated module documentation**](./docs/typedocs/modules.md)
    - [`components` module](./docs/typedocs/modules.md#modules) (UI components)
    - [`config` module](./docs/typedocs/config/README.md) (BYTM feature configuration)
    - [`constants` module](./docs/typedocs/constants/README.md) (BYTM-related constants)
    - [`interface` module](./docs/typedocs/interface/README.md) (plugin interface functions and events)
    - [`types` module](./docs/typedocs/types/README.md) (TypeScript types for most of BYTM)
    - [`siteEvents` module](./docs/typedocs/siteEvents/README.md) (events related to changes on the site)
...
These are the global functions, constants and classes that are exposed by BetterYTM through the `unsafeWindow.BYTM` object.  
...
> [!IMPORTANT]  
> 
> - [The `unsafeWindow` GreaseMonkey API](https://wiki.greasespot.net/unsafeWindow) is used here because it is important that the [`window` object](https://developer.mozilla.org/en-US/docs/Web/API/Window) is not sandboxed.  
>   For regular browser extensions, `window` or [`globalThis`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/globalThis) will be sufficient and can be used instead.
> - Authenticated functions are marked with 🔒 and need to be passed a per-session and per-plugin authentication token.  
>   It can be acquired by calling [`registerPlugin()`](#registerplugin)

<br>
  
- Meta:
  - [`function registerPlugin()`](#registerplugin) - Registers a plugin with BetterYTM with the given plugin definition object
  - [`function getPluginInfo()`](#getplugininfo) 🔒 - Returns the plugin info object for the specified plugin - can be used to check if a certain plugin is registered
  - [`function getInternals()`](#getinternals) 🔒 - Returns functions and instances useful for core libraries or deeper-reaching plugins
- BYTM-specific:
  - [`function getDomain()`](#getdomain) - Returns the current domain of the page as a constant string (either "yt" or "ytm")
  - [`function getResourceUrl()`](#getresourceurl) - Returns a `blob:` URL provided by the local userscript extension for the specified BYTM resource file
  - [`function resourceAsString()`](#resourceasstring) - Returns the specified resource as a string - uses a cross-session cache for better performance
  - [`function getSessionId()`](#getsessionid) - Returns the unique session ID that is generated on every started session
  - [`function reloadTab()`](#reloadtab) - Reloads the current tab while preserving video time and volume and making features like initial tab volume lower priority
- DOM:
  - [`function getBytmDialog()`](#getbytmdialog) - A class for creating and managing modal, fully customizable dialogs
  - [`function getExImDialog()`](#geteximdialog) - Subclass of BytmDialog for allowing users to export and import serializable data
  - [`function getMarkdownDialog()`](#getmarkdowndialog) - Subclass of BytmDialog for displaying markdown content
  - [`function setInnerHtml()`](#setinnerhtml) - Sets the innerHTML property of an element after sanitizing the string with [DOMPurify](https://github.com/cure53/DOMPurify) and [Trusted Types](https://developer.mozilla.org/en-US/docs/Web/API/Trusted_Types_API)
  - [`function addSelectorListener()`](#addselectorlistener) - Adds a listener that checks for changes in DOM elements matching a CSS selector
  - [`function onInteraction()`](#oninteraction) - Adds accessible event listeners to the specified element for button or link-like keyboard and mouse interactions
  - [`function getVideoTime()`](#getvideotime) - Returns the current video time (on both YT and YTM)
  - [`function getThumbnailUrl()`](#getthumbnailurl) - Returns the URL to the thumbnail of the currently playing video
  - [`function getBestThumbnailUrl()`](#getbestthumbnailurl) - Returns the URL to the best quality thumbnail of the currently playing video
  - [`function fetchITunesAlbumInfo()`](#fetchitunesalbuminfo) - Fetches the Apple Music / iTunes album info objects for the specified artist and album name
  - [`function waitVideoElementReady()`](#waitvideoelementready) - Waits for the video element to be queryable in the DOM - has to be called after `bytm:observersReady`
  - [`function getVideoElement()`](#getvideoelement) - Returns the video element on the current page or null if there is none (on both YT and YTM)
  - [`function getVideoSelector()`](#getvideoselector) - Returns the CSS selector for the video element (on both YT and YTM)
  - [`function getCurrentMediaType()`](#getcurrentmediatype) - (On YTM only) returns the type of media that is currently playing (either "video" or "song")
  - [`function getLikeDislikeBtns()`](#getlikedislikebtns) - Returns the like and dislike buttons for either domain, as well as the current like/dislike state
  - [`function isIgnoredInputElement()`](#isignoredinputelement) - Checks if the given element (or `document.activeElement`) is an input element that should prevent all other keypresses from being processed
- Site Events:
  - [`function onSiteEvent()`](#onsiteevent) - Adds a site event listener
  - [`function onceSiteEvent()`](#oncesiteevent) - Adds a site event listener that is only called once and also returns a Promise for use with the async/await pattern
  - [`function onMultiSiteEvents()`](#onmultisiteevents) - Adds a listener for multiple site events at once, with configurable behavior and with a shared callback function
- Components:
  - [`function createHotkeyInput()`](#createhotkeyinput) - Creates a hotkey input element
  - [`function createToggleInput()`](#createtoggleinput) - Creates a toggle input element
  - [`function createCircularBtn()`](#createcircularbtn) - Creates a generic, circular button element with just an icon
  - [`function createLongBtn()`](#createlongbtn) - Creates a generic, long and circular button element with an icon and text
  - [`function createRipple()`](#createripple) - Creates a click ripple effect on the given element
  - [`function showToast()`](#showtoast) - Shows a toast notification and a message string or element
  - [`function showIconToast()`](#showicontoast) - Shows a toast notification with an icon and a message string or element
  - [`function showPrompt()`](#showprompt) 🔒 - Promise-based styled modal dialog mimicking the browser-native [`confirm()`](https://developer.mozilla.org/en-US/docs/Web/API/Window/confirm), [`alert()`](https://developer.mozilla.org/en-US/docs/Web/API/Window/alert) and [`prompt()`](https://developer.mozilla.org/en-US/docs/Web/API/Window/prompt)
- Translations:
  - [`function setLocale()`](#setlocale) 🔒 - Sets the locale for BetterYTM
  - [`function getLocale()`](#getlocale) - Returns the currently set locale
  - [`function hasKey()`](#haskey) - Checks if the specified translation key exists in the currently set locale
  - [`function hasKeyFor()`](#haskeyfor) - Checks if the specified translation key exists in the specified locale
  - [`function t()`](#t) - Translates the specified translation key using the currently set locale
  - [`function tp()`](#tp) - Translates the specified translation key including pluralization using the currently set locale
  - [`function tl()`](#tl) - Returns the translation for the provided key and provided locale
  - [`function tlp()`](#tlp) - Returns the translation for the provided locale and key, including pluralization identifier
- Feature config:
  - [`function getFeatures()`](#getfeatures) 🔒 - Returns the current BYTM feature configuration object
  - [`function saveFeatures()`](#savefeatures) 🔒 - Overwrites the current BYTM feature configuration object with the provided one
  - [`function getDefaultFeatures()`](#getdefaultfeatures) - Returns the default feature configuration object
- Lyrics:
  - [`function fetchLyricsUrlTop()`](#fetchlyricsurltop) - Fetches the URL to the lyrics page for the specified song
  - [`function getLyricsCacheEntry()`](#getlyricscacheentry) - Tries to find a URL entry in the in-memory cache for the specified song
  - [`function sanitizeArtists()`](#sanitizeartists) - Sanitizes the specified artist string to be used in fetching a lyrics URL
  - [`function sanitizeSong()`](#sanitizesong) - Sanitizes the specified song title string to be used in fetching a lyrics URL
- Auto-Like:
  - [`function getAutoLikeData()`](#getautolikedata) 🔒 - Returns the current auto-like data object
  - [`function saveAutoLikeData()`](#saveautolikedata) 🔒 - Overwrites the current auto-like data object with the provided one
  - [`function fetchVideoVotes()`](#fetchvideovotes) - Fetches the approximate like and dislike count for the video with the specified ID
- Logging:
  - [`const loggers`](#loggers) - Object of predefined Logger instances for different categories.
  - [`class Logger`](#logger) - Class for creating new logging categories. Will automatically share logs with BYTM's logging system.
- Other:
  - [`class NanoEmitter`](#nanoemitter) - Abstract class for creating lightweight, type safe event emitting classes
  - [`function formatNumber()`](#formatnumber) - Formats a number with the configured locale and passed or configured format
...
> - ⚠️ When the tab is duplicated, the session ID will also be duplicated and could lead to unexpected behavior.
>   
> <details><summary><b>Example <i>(click to expand)</i></b></summary>
> 
> ```ts
...
> Works on both YouTube and YouTube Music, as well as with the YT Shorts player.  
...
> Signature:
> ```ts
> const loggers: Record<LooseUnion<LogCategory>, Logger>;
> ```
>   
> BetterYTM provides a set of loggers that can be used to log messages with different categories, so they may be filtered and styled differently.  
> A set of predefined categories is defined in the `type LogCategory`, but since it's a loose union type, there may be additional categories that are not listed there but can still be used as keys in the `loggers` object.  
> For more information on the contained Logger instances, refer to [the docs for the `class Logger`.](#class-logger)  
>   
> <details><summary><b>Example <i>(click to expand)</i></b></summary>
> 
> ```ts
> const { loggers } = unsafeWindow.BYTM;
> 
> async function run() {
>   try {
>     const data = await fetchPluginData();
>     loggers.data.log("Fetched plugin data:", data);
>   }
>   catch(err) {
>     loggers.data.error("Failed to fetch plugin data due to error:", err);
>   }
> 
>   try {
>     const element = await addMyComponentToPage();
>     loggers.layout.log("Added component to page:", element); 
>   }
>   catch(err) {
>     loggers.layout.error("Failed to add component to page due to error:", err);
>   }
> }
> 
> run();
...
> Signature:
> ```ts
> const myLogger = new unsafeWindow.BYTM.Logger(category: string, options?: LoggerOptions);
> ```
>   
> Logger class that is used to create loggers for different categories, so that the logs can be filtered and styled differently based on their category.  
> The category is just a string that is used as a label for the logger and can be anything. The convention is to use PascalCase for categories.  
> Every instance will automatically share its collected logs with BYTM's logging system, so that they will be included in the log file downloaded via userscript manager extension menu command.  
>   
> Options:
> - `onError?: (...args: unknown[]) => void` - Optional callback function that gets called when the method `error()` is called. Used in BYTM for showing toast notifications.
>   
> Methods:
> - `log(...args: unknown[]): void` - Logs the passed arguments as a regular log message - like `console.log()`
> - `error(...args: unknown[]): void` - Logs the passed arguments as an error message - like `console.error()`. Also triggers the `onError` callback if it was provided in the constructor options. The first passed `Error` argument will be used as the main error for the log entry.
> - `warn(...args: unknown[]): void` - Logs the passed arguments as a warning message - like `console.warn()`
> - `dbg(...args: unknown[]): void` - Logs the passed arguments as a debug message, only visible when the log level is set to Debug (1)
>   
> Static Methods:
> - `pushLog(category: string, type: string, time?: number, ...args: unknown[])` - Called to push a new log entry to the global log store. The predefined logging methods will already call this internally.
> - `serializeLogVal(val: unknown, primaryScope?: boolean)` - Used to serialize any value to a string for logging purposes. Called internally when pushing a log entry.
> - `getLogLevel(args: unknown[]): LogLevel` - Parses the given arguments to determine if a log level was passed. Returns Info (0) by default.
> - `serializeLogs()` - Returns a string representation of all logs in the global log store, for downloading as a log file.
>   
> <details><summary><b>Example <i>(click to expand)</i></b></summary>
> 
> ```ts
> const myLogger = new unsafeWindow.BYTM.Logger("MyCategory", {
>   onError: (...args) => {
>     // show a toast notification for every error logged with this logger:
>     unsafeWindow.BYTM.showToast({
>       message: "An error occurred in MyCategory!",
>       duration: 5_000,
>     });
>   },
> });
>
> myLogger.log("This is a regular log message with some data:", { foo: "bar" }, 123, -Infinity);
> myLogger.warn("This is a warning message about something:", { baz: "qux" });
> myLogger.error("This is an error message with an error object:", new Error("Something went wrong", { cause: new TypeError("Something else") }));
> myLogger.dbg("This is a debug message that is only visible in the log file when the log level is set to Debug");
> 
> function getLogs(): string {
>   return unsafeWindow.BYTM.Logger.serializeLogs();
> }
> ```
